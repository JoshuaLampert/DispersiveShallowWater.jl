...
function create_gif(filename; ylims = :auto, kwargs...)
  trixi_include(joinpath(examples_dir(), filename); kwargs...)
  x = DispersiveShallowWater.grid(semi)
  anim = @animate for step in 1:length(sol.u)
    t = sol.t[step]
    plot(x, view(sol.u[step], 1, :), legend = true, ylims = ylims, label = "approximation",
         title = "time t=$(round(t, digits=5))")
    analytical_sol = DispersiveShallowWater.compute_coefficients(initial_condition, solver,
                                                                 t, equations, mesh)
...
create_gif("bbm_bbm_1d_basic.jl"; ylims = (-8, 4), tspan = (0.0, 50.0))
